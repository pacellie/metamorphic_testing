    ...
    def deco_sut(func):
        def execute_all(*args):
            ...
            for i, s in enumerate(suite, 1):  # run each test
                ...
                def inner(**kwargs):
                    first, *others = kwargs.values()  # retrieve the first arg (the anchor)
                    test_function = func
                    transformation_function = suite[s].transformation
                    relation_function = suite[s].relation
